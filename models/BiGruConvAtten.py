...
class BiGruConvAtten(Network):
    def __init__(self):
        Network.__init__(self)

    def build(self, embedding_matrix=np.array([None])):
        print('Build Bidirectional GRU add Conv model based on Attention...')
        self.set_name("BiGruConvAtten")
        ...
        atten = Conv1D(filters=1, kernel_size=1)(x)
        atten = Activation('softmax')(atten)
        x = multiply([x, atten])
        x = Conv1D(filters=250, kernel_size=3)(x)
